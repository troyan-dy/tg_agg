...
📝 Сменить ленту, 🕒 Часы, ❓ Помощь). Кнопка «📝 Сменить ленту» запускает
диалог — бот ждёт ссылку следующим сообщением. Кнопка «🕒 Часы» открывает
сетку 0–23 с инлайн-кнопками: тап включает/выключает час (✅/▫️), изменения
применяются сразу. Те же действия продублированы в меню «/».

При каждом запуске сервиса админу приходит сообщение «♻️ Сервис перезапущен»
с текущими настройками (канал, лента, часы, модель) — чтобы тихий рестарт после
сбоя не остался незамеченным.
...
├── models.py            # Setting (rss_url, run_hours) и SeenItem (дедуп)
├── storage.py           # доступ к данным: rss-url, часы, дедуп, отметки
