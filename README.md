...
2. Из базы поднимаются **заголовки, опубликованные за последние 24 часа**
   (`storage.recent_published_titles` — записи с `published=true` и `posted_at`
   в пределах суток, новые первыми).
3. DeepSeek (`deepseek.pick_most_relevant`) получает оба списка — уже
...
4. Ответ — индекс выбранной новости (строгий JSON). При сбое парсинга берётся
